...
export class FriendService {
    ...
    async getFriends(userId: string) {
        ...
        return friends.map(fr => fr.senderId === userId ? fr.receiver : fr.sender)
    }
    ...
    async getFriendsEvents(userId: string) {
        ...
        const promises = friendsIds.map(async friendId => await this.eventService.getByUserId(friendId))
        ...
        const filteredDateEvents = events.filter(item => new Date(String(item?.date)) > new Date(isoDateString))

        filteredDateEvents.sort((a, b) => new Date(String(a?.date)).getTime() - new Date(String(b?.date)).getTime());
